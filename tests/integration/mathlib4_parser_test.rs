...
fn test_parse_mathlib4_files() {
    ...
    assert!(mathlib_path.exists(), "mathlib4 test data not found. Run: git submodule update --init --recursive");
    ...
    assert!(success_rate > 0.0, "Parser should be able to parse at least some files");
}
