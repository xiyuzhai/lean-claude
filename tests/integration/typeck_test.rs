...
fn test_type_inference_application() {
    ...
    // For now, test with unbound identifiers
...
fn test_unification_basic() {
    ...
    let m1 = mctx.mk_metavar(
        Expr::sort(Level::zero()),
        lctx.clone(),
    );
    let m2 = mctx.mk_metavar(
        Expr::sort(Level::zero()),
        lctx.clone(),
    );
...
fn test_whnf_reduction() {
    ...
    let plus_one = Expr::app(
        Expr::app(
            Expr::const_expr("Nat.add".into(), vec![]),
            Expr::bvar(0),
        ),
        Expr::nat(1),
    );
