...
namespace Syntax.Patterns.Advanced
open TestPrelude
...
def replicate {α : Type} : (n : Nat) → α → Vector α n
  | 0, _ => Vector.nil
...
def isJust {α : Type} : Option α → Bool
  | some _ => true
  | none => false

def fromJust {α : Type} : (o : Option α) → isJust o = true → α
  | some x, _ => x
