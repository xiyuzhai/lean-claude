...
macro "try!" x:term : term => `(match $x with | some v => v | none => panic! "unwrap failed")
...
-- Usage:
def piped : Nat := 5 |> double |> triple
-- Expands to: def piped := triple (double 5)
-- Which expands to: def piped := triple (5 + 5)
-- Which expands to: def piped := (5 + 5) + (5 + 5) + (5 + 5)

-- Example 8: TODO macro
macro "todo!" msg:interpolatedStr(term) : term => `(panic! s!"TODO: {$msg}")
...
macro "with_temp" val:term "in" body:term : term => `(let tmp := $val; $body)

-- Usage:
def with_temp_example : Nat := with_temp 42 in tmp + 1
